...
/* This function correctly reverses the elements of a queue.反转一个队列
 */
...
//        return 0;
//    }
    s.push(0);//for the empty situation
    int total = s.pop();
    while (!s.isEmpty()) {
        total += s.pop();
    }
...
/*
 * the following code is Student_TEST
 */

STUDENT_TEST("duplicateNegatives") {
     //Queue<int> q = {-3, 5, 10};
     //duplicateNegatives(q);
     //EXPECT_EQUAL(q, {-3,-3,5,10});

//     Queue<int> w = {3, 5, -10};
//     duplicateNegatives(w);
//     EXPECT_EQUAL(w, {3,5,-10,-10});

     Queue<int> w = {-3,5,10,10};
     duplicateNegatives(w);
     EXPECT_EQUAL(w, {-3,-3,5,10,10});
